...
class Post(models.Model):
    ...
    amazon_link = models.URLField(blank=True, null=True, max_length=8000)
    amazon_product_name = models.CharField(max_length=255, blank=True, null=True)
    amazon_product_image_url = models.URLField(max_length=8000, blank=True, null=True)
...
class LinkClick(models.Model):
    ...
    def __str__(self):
        return f"Click on post {self.post.id} at {self.created_at}"
...
class ReportPost(models.Model):
    ...
    def __str__(self):
        return f"{self.user.username} reported post {self.post.id}"